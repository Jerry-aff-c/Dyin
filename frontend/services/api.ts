...
export const api = {
  ...
  monitor: {
    /** 获取监控状态 */
    status: () => get<any>('/api/monitor/status'),
    
    /** 开始监控任务 */
    start: (cookie: string = '') => post<any>('/api/monitor/start', { cookie }),
    
    /** 停止监控任务 */
    stop: () => post<any>('/api/monitor/stop'),
    
    /** 获取监控数据 */
    data: () => get<any>('/api/monitor/data'),
    
    /** 设置抖音sec_user_id */
    setSecUserId: (sec_user_id: string) => post<any>('/api/monitor/set-sec-user-id', { sec_user_id }),
    
    /** 激活授权码 */
    activate: (license_key: string) => post<any>('/api/monitor/auth/activate', { license_key }),
    
    /** 获取授权状态 */
    authStatus: () => get<any>('/api/monitor/auth/status'),
  },
};
